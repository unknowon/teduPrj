...
<head>
    ...
    <!-- DataTables -->
    <link rel="stylesheet" href="<%=ctxPath%>/css/datatables.net-bs/dataTables.bootstrap.min.css">
    <!-- Theme style -->
            ...
            <ul class="sidebar-menu" data-widget="tree">
                ...
                <i class="fa fa-angle-left pull-right"></i>
                    ...
                    <ul class="treeview-menu">
                        <li <c:if test="${sta == null}">class="active"</c:if> > <a href="<%=ctxPath%>/Order/orderList.do">所有订单</a></li>
                        <li <c:if test="${sta eq '待处理'}">class="active"</c:if>><a href="<%=ctxPath%>/Order/orderList.do?sta=待处理">待处理</a></li>
                        <li <c:if test="${sta eq '处理中'}">class="active"</c:if>><a href="<%=ctxPath%>/Order/orderList.do?sta=处理中">处理中</a></li>
                        <li <c:if test="${sta eq '已发货'}">class="active"</c:if>><a href="<%=ctxPath%>/Order/orderList.do?sta=已发货">已发货</a></li>
                        <li <c:if test="${sta eq '已交付'}">class="active"</c:if>><a href="<%=ctxPath%>/Order/orderList.do?sta=已交付">已交付</a></li>
                    </ul>
                </li>
                <li class="treeview">
            ...
            <div class="box box-primary">
                <div class="box-body">
                    <table id="my_order" class="table table-bordered table-striped">
                        ...
                        <tr>
                            <td>${order.id}</td>
                            <td><span class="label <c:if test="${order.sta eq '待处理'}">bg-yellow</c:if><c:if test="${order.sta eq '处理中'}">bg-aqua</c:if><c:if test="${order.sta eq '已发货'}">bg-green</c:if><c:if test="${order.sta eq '已交付'}">bg-red</c:if>">${order.sta}</span></td>
                            <td><fmt:formatDate value="${order.placed}" pattern="yyyy-MM-dd HH:mm"/></td>
                            <td>￥${order.payment}</td>
                            <td><a href="<%=ctxPath%>/Order/orderDetail.do?id=${order.id}">查看</a>丨<a href="<%=ctxPath%>/Order/orderProcess.do?id=${order.id}">处理</a></td>
                        </tr>
                        </c:forEach>
...
<!-- AdminLTE App -->
<script src="<%=ctxPath%>/js/AdminLTE/adminlte.min.js"></script><!-- moment -->
<!-- <script src="<%=ctxPath%>/js/moment/moment.min.js"></script> -->
<!-- page script -->
<script>
    $(function () {
        $('#my_order').DataTable({
     ...
     'processing': true,
     'serverSide': true,
     'ajax': '<%=ctxPath%>/servlet/all-order',
     'columns': [
    	    ...
    	    { 'data': 'sta' , 'render': function (data, type, row, meta) {
    	    	var content = '<a href="<%=ctxPath%>/servlet/order-detail?orderId='+row.orderId+'">查看</a>丨';
    	    	if(data=='已交付'){
    	    		content+='处理';
    	    	}else{
    	    		content+='<a href="<%=ctxPath%>/servlet/order-process?orderId='+row.orderId+'">处理</a>';
    	    	}
    	    	return content;
	    	  }
